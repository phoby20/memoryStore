...
server.tool(
  "get_memories",
  "사용자의 장기 메모리를 가져옵니다. 카테고리로 필터링 가능.",
  { category: z.string().optional().describe("필터링할 카테고리 (취향, 건강, 인간관계, 재무, 목표, 습관, 기타)") },
  async ({ category }) => {
    ...
    const text = data.memories
      .map((m: { category: string; key: string; value: string }) => `[${m.category}] ${m.key}: ${m.value}`)
      .join("\n");
...
server.tool(
  "save_memory",
  "사용자에 대해 학습한 정보를 장기 메모리에 저장합니다. 대화 중 사용자의 취향, 습관, 목표 등을 발견하면 저장하세요.",
  {
...
server.tool(
  "delete_memory",
  "특정 메모리를 삭제합니다.",
  { id: z.string().describe("삭제할 메모리 ID") },
  async ({ id }) => {
